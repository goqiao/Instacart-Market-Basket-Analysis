...
substitute = substitute.loc[substitute['similarity_score'] > 0.8, ['product_id', 'substitute_id']]
...
user_substitute = user_substitute.groupby(['user_id', 'product_id']).agg({'up_word2vec_substitute_num_purchases':'sum',
                                                                          'up_word2vec_substitute_num_purchases_r5':'sum'}).reset_index()


user_substitute.to_pickle('{}/up_word2vec_substitute_purchase.pickle'.format(data_folder))
