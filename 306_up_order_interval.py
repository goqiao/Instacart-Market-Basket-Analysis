...
up_orders["order_interval"] = up_orders.groupby(["user_id", "product_id"])[
    "order_number"
].diff()

up_orders = up_orders.loc[~up_orders["order_interval"].isnull()]
up_orders_interval = up_orders.groupby(["user_id", "product_id"]).agg(
    {"order_interval": [list, "mean", "max", "min", "median", trend_d1, skewness]}
)
