...
orders["order_number_reverse"] = (
    orders.groupby("user_id")["order_number"].transform(np.max)
    + 1
    - orders["order_number"]
)
...
prior_order_details = orders[
    ["order_id", "user_id", "order_number", "days_since_prior_order"]
].merge(prior, on="order_id", how="inner")
prior_order_details["_up_purchase_order"] = prior_order_details.groupby(
    ["user_id", "product_id"]
)["order_number"].rank()
prior_order_details["last_nth_order"] = (
    prior_order_details.groupby("user_id")["order_number"].transform(np.max)
    + 1
    - prior_order_details["order_number"]
)
prior_order_details["user_max_order"] = prior_order_details.groupby("user_id")[
    "order_number"
...
base = base.merge(
    prior_order_details[["user_id", "product_id"]].drop_duplicates(), on="user_id"
)

base = base.merge(
    train[["order_id", "product_id", "reordered"]], on="order_id", how="left"
)
...
base.loc[base["eval_set"] == "train", "reordered"] = base.loc[
    base["eval_set"] == "train", "reordered"
].fillna(0)
...
user_orders = prior_order_details[
    ["user_id", "order_number", "days_since_prior_order"]
].drop_duplicates()
user_orders.sort_values(by=["user_id", "order_number"], inplace=True)
user_orders["nth_day_since_customer"] = (
    user_orders.groupby("user_id")["days_since_prior_order"].cumsum().fillna(0)
)
...
_up = prior_order_details[col + ["order_id"]].merge(
    user_orders, on=["user_id", "order_number"]
)
gc.collect()
...
_up["_up_days_since_last_purchase"] = (
    _up["nth_day_since_customer"]
...
order_prods = (
    prior.groupby("order_id").apply(lambda x: x["product_id"].to_list()).reset_index()
)
...
products_sub_pairs = products.loc[
    products["product_organic_name"].isin(products["product_name"]),
    ["product_id", "product_name", "product_organic_name"],
]
products_sub_pairs.columns = ["product_id", "product_name", "substitute_product_name"]
products_sub_pairs = products_sub_pairs.merge(
...
products_sub_pairs = products_sub_pairs[
    ["product_id", "product_name", "substitute_id", "substitute_product_name"]
]
...
products["p_is_gluten_free"] = (
    products["product_name"].str.lower().str.contains("gluten")
