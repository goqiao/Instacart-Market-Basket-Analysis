...
substitute = pd.read_pickle('data/products_organic_substitution.pickle')[['product_id', 'substitute_id']]
if two_way:
    substitute_rev = pd.read_pickle('data/products_organic_substitution.pickle')[['product_id', 'substitute_id']].rename(
...
print(user_substitute.loc[(user_substitute.user_id==91812) & (user_substitute.product_id==31915)])
...
# up_purchase = up_purchase.merge(products, how='inner')
up_purchase.columns = ['user_id', 'substitute_id', 'up_organic_substitute_num_purchases']
...
up_purchases_r5.columns = ['user_id', 'substitute_id', 'up_organic_substitute_num_purchases_r5']
...
user_substitute = user_substitute.groupby(['user_id', 'product_id']).agg({'up_organic_substitute_num_purchases':'sum',
                                                                          'up_organic_substitute_num_purchases_r5':'sum'}).reset_index()
# print(user_substitute.loc[user_substitute.user_id == 111429 ].head())

print(user_substitute[['up_organic_substitute_num_purchases', 'up_organic_substitute_num_purchases_r5']].describe())
print(user_substitute.isnull().sum())
user_substitute.to_pickle('{}/up_organic_substitute_purchase.pickle'.format(data_folder))
