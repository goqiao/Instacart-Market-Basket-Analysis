...
prior_order_details["up"] = (
    prior_order_details["user_id"].astype("str")
    + " "
    + prior_order_details["product_id"].astype("str")
)

dow = pd.crosstab(
    prior_order_details["up"], prior_order_details["order_dow"]
).add_prefix("up_purchases_dow_")
dow_norm = pd.crosstab(
    prior_order_details["up"], prior_order_details["order_dow"], normalize="index"
).add_prefix("up_norm_purchases_dow_")

part_of_day = pd.crosstab(
    prior_order_details["up"], prior_order_details["part_of_day"]
).add_prefix("up_purchases_pod_")
part_of_day_norm = pd.crosstab(
    prior_order_details["up"], prior_order_details["part_of_day"], normalize="index"
).add_prefix("up_norm_purchases_pod_")

up_order_time = pd.concat(
...
up_order_time[["user_id", "product_id"]] = up_order_time[
    ["user_id", "product_id"]
].astype("int")
