...
def make_data(data_folder='data', make_set='train'):
    ...
    base = pd.read_pickle('{}/base.pickle'.format(data_folder)).set_index(index_cols)

    # read pre-created features
    up_agg = pd.read_pickle('{}/up_agg.pickle'.format(data_folder)).set_index(index_cols)
    up_purchase_r5 = pd.read_pickle('{}/up_purchase_r5.pickle'.format(data_folder)).set_index(index_cols)
    up_days_since_last_purchase = pd.read_pickle('{}/up_days_since_last_purchase.pickle'.format(data_folder)).set_index(index_cols)
    up_days_since_last_purchase_r5 = pd.read_pickle('{}/up_days_since_last_purchase_r5.pickle'.format(data_folder)).set_index(index_cols)
    up_purchase_proba = pd.read_pickle('{}/up_purchase_proba.pickle'.format(data_folder)).set_index(index_cols)
    up_purchase_proba_r5 = pd.read_pickle('{}/up_purchase_proba_r5.pickle'.format(data_folder)).set_index(index_cols)
    up_days_not_purchase = pd.read_pickle('{}/up_days_not_purchase.pickle'.format(data_folder)).set_index(index_cols)
    up_aisles_purchase_trend = pd.read_pickle('{}/up_aisles_purchase_trend.pickle'.format(data_folder)).set_index(index_cols)
    up_departments_purchase_trend = pd.read_pickle('{}/up_departments_purchase_trend.pickle'.format(data_folder)).set_index(index_cols)
    up_substitute_purchase = pd.read_pickle('{}/up_substitute_purchase.pickle'.format(data_folder)).set_index(index_cols)
    up_organic_substitute_purchase = pd.read_pickle('{}/up_organic_substitute_purchase.pickle'.format(data_folder)).set_index(index_cols)
    up_order_time = pd.read_pickle('{}/up_order_time.pickle'.format(data_folder)).set_index(index_cols)
    ...
    index_cols = ['user_id']
    users_features = pd.read_pickle('{}/users_features.pickle'.format(data_folder)).set_index(index_cols)
    users_basket_size_trend = pd.read_pickle('{}/users_basket_size_trend.pickle'.format(data_folder)).set_index(index_cols)
    users_orders_interval_trend = pd.read_pickle('{}/users_orders_interval_trend.pickle'.format(data_folder)).set_index(index_cols)
    users_organic_purchases = pd.read_pickle('{}/users_organic_purchases.pickle'.format(data_folder)).set_index(index_cols)
    users_organic_purchases_r5 = pd.read_pickle('{}/users_organic_purchases_r5.pickle'.format(data_folder)).set_index(index_cols)
    users_order_time = pd.read_pickle('{}/users_order_time.pickle'.format(data_folder)).set_index(index_cols)
    # users_cluster = pd.read_pickle('{}/user_kmeans_cluster.pickle'.format(data_folder)).set_index(index_cols)

    index_cols = ['product_id']
    product_features_basic_agg = pd.read_pickle('{}/product_features_basic_agg.pickle'.format(data_folder)).set_index(index_cols)
    product_organic_features = pd.read_pickle('{}/product_organic_features.pickle'.format(data_folder)).set_index(index_cols)
    products_purchases_features = pd.read_pickle('{}/products_purchases_features.pickle'.format(data_folder)).set_index(index_cols)
    product_purchase_cycle = pd.read_pickle('{}/product_purchase_cycle.pickle'.format(data_folder)).set_index(index_cols)
    products_embedding = pd.read_pickle('{}/product_embedding.pickle'.format(data_folder)).set_index(index_cols)
    # product_sub_stats = pd.read_pickle('{}/product_sub_stats.pickle'.format(data_folder)).set_index(index_cols)
    ...
    # data_full_features = data_full_features.join(users_features[selected_users_features], on='user_id', how='inner')
    ...
    # release memory
    gc.collect(); del up_purchase_proba, up_purchase_proba_r5, up_days_not_purchase, users_features, \
        users_basket_size_trend, users_orders_interval_trend, users_organic_purchases, users_organic_purchases_r5, users_order_time

    data_full_features = improve_data_type(data_full_features)
    print(data_full_features.head(1))
    ...
    data_full_features = data_full_features.join(product_organic_features, on='product_id', how='left')
    ...
    print('finish creating {} set'.format(make_set))
