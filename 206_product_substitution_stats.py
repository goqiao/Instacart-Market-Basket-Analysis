...
sub_stats1 = sub.groupby("product_id").agg(
    {"substitute_id": "nunique", "similarity_score": "max"}
)
sub_stats1.columns = ["p_num_substitute", "p_max_similarity_score"]


sub["similarity_score_bucket"] = pd.cut(
    sub["similarity_score"], right=False, bins=[0.5, 0.6, 0.7, 0.8, 0.9, 1]
)
sub_stats2 = pd.crosstab(
    index=sub["product_id"], columns=sub["similarity_score_bucket"]
).add_prefix("p_num_products_sim_score")

sub_stats = pd.concat([sub_stats1, sub_stats2], axis=1)
# rename as special characters, '[, )' are not supported in xgboost
sub_stats.rename(
    columns={
        "p_num_products_sim_score[0.5, 0.6)": "p_num_products_sim_score_0.5_0.6",
