...
users_aisles_cnts = pd.crosstab(index=prior_order_details['user_id'], columns=prior_order_details['aisle']).reset_index()
...
user_cluster_res = pd.get_dummies(users[['user_id', 'user_cluster']], prefix='user_cluster', columns=['user_cluster'])
