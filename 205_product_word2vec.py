...
train_products = (
    train.groupby("order_id")["product_id"].apply(lambda x: x.tolist()).reset_index()
)
prior_products = (
    prior.groupby("order_id")["product_id"].apply(lambda x: x.tolist()).reset_index()
)
...
prods_embedding = pd.DataFrame(
    index=model.wv.key_to_index.keys(), data=model.wv.vectors
...
product_embedding = (
    pd.DataFrame(index=prods_embedding.index, data=pc, columns=np.arange(pc.shape[1]))
    .add_prefix("p_embedding_")
    .reset_index()
)
