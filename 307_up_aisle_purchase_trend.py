...
users_aisles = (
    prior.groupby(["user_id", "aisle", "order_number"])
...
df = (
    users_aisles[["user_id", "aisle"]]
    .drop_duplicates()
    .merge(users_orders, on="user_id", how="outer")
)
users_aisles = df.merge(
    users_aisles, how="left", on=["user_id", "aisle", "order_number"]
).fillna(0)
del df
gc.collect()

users_aisles_trend = (
    users_aisles.groupby(["user_id", "aisle"])
    .agg({"num_products": ["mean", trend_d1, skewness]})
    .reset_index()
)
users_aisles_trend.columns = [
    "user_id",
    "aisle",
    "up_mean_prods_same_aisle_per_order",
...
].to_pickle("{}/up_aisles_purchase_trend.pickle".format(data_folder))
