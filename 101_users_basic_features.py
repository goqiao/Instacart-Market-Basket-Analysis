...
users_1 = (
    orders.loc[orders["eval_set"] == "prior"]
    .groupby("user_id")
    .agg(
        {
...
users_1.columns = (
    users_1.columns.get_level_values(0) + "_" + users_1.columns.get_level_values(1)
)
...
users_1.rename(
    columns={
        ...
        "order_hour_of_day_std": "user_std_order_hour",
    },
    inplace=True,
)
...
users_2 = prior_order_details.groupby("user_id").agg(
    {
...
users_2.columns = (
    users_2.columns.get_level_values(0) + "_" + users_2.columns.get_level_values(1)
)

users_2.rename(
    columns={
        "product_id_count": "user_product_total",
...
users_2 = users_2.merge(
    orders.loc[orders["eval_set"] != "prior", ["user_id", "days_since_prior_order"]],
    how="left",
).rename(columns={"days_since_prior_order": "user_days_not_purchase"})

## aggregate on users_orders(uo) level
users_3_temp = prior_order_details.groupby(["user_id", "order_id"]).agg(
    {
        "product_id": ["count"],
        "aisle": ["nunique"],
        "department": ["nunique"],
...
users_3_temp.columns = (
    users_3_temp.columns.get_level_values(0)
    + "_"
    + users_3_temp.columns.get_level_values(1)
)

users_3 = (
    users_3_temp.reset_index()
    .drop(columns="order_id")
    .groupby("user_id")
    .agg(["mean", "std"])
)
users_3.columns = (
    users_3.columns.get_level_values(0) + "_" + users_3.columns.get_level_values(1)
)

users_3.rename(
    columns={
        "product_id_count_mean": "uo_basket_size_mean",
        ...
        "reordered_mean_std": "uo_reorder_ratio_std",
    },
    inplace=True,
)
...
users_features["user_reorder_rate"] = (
    users_features["user_reorder_prod_total"] / users_features["user_product_total"]
)

users_features["user_next_order_readiness"] = (
    users_features["user_days_not_purchase"]
    - users_features["user_mean_days_order_interval"]
)
users_features["user_order_freq_days_mean"] = (
    users_features["user_age_days_on_platform"] / users_features["user_total_orders"]
)
