...
up["up_purchase_interval_days_median"] = up_purchase_time.groupby(key)[
    "_up_days_since_last_purchase"
].median()
up["up_purchase_interval_days_max"] = up_purchase_time.groupby(key)[
    "_up_days_since_last_purchase"
].max()
up["up_purchase_interval_days_min"] = up_purchase_time.groupby(key)[
    "_up_days_since_last_purchase"
].min()
...
key = ["user_id", "product_id"]
up_r5 = (
    up_purchase_time_r5.groupby(key)["_up_days_since_last_purchase"].median().to_frame()
)
up_r5.columns = ["up_purchase_interval_days_median_r5"]

up_r5["up_purchase_interval_days_max_r5"] = up_purchase_time_r5.groupby(key)[
    "_up_days_since_last_purchase"
].max()
up_r5["up_purchase_interval_days_min_r5"] = up_purchase_time_r5.groupby(key)[
    "_up_days_since_last_purchase"
].min()

up_r5.reset_index().to_pickle(
    "{}/up_days_since_last_purchase_r5.pickle".format(data_folder)
)
