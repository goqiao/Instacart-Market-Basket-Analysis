...
up_pred_prob = pd.read_pickle("data/val_pred_prob_res.pickle")[
    ["order_id", "product_id", "pred_proba", "reordered"]
]
...
if __name__ == "__main__":
    ...
    val_pred_reorder_long = pd.melt(
        pd.concat(
            [up_prob["order_id"], up_prob["products"].str.split(expand=True)], axis=1
        ),
        id_vars="order_id",
        value_name="product_id",
    ).drop("variable", axis=1)

    val_pred_reorder_long = val_pred_reorder_long.loc[
        val_pred_reorder_long.product_id.apply(lambda x: x is not None)
        & (val_pred_reorder_long.product_id != "None")
    ]
    val_pred_reorder_long["product_id"] = val_pred_reorder_long["product_id"].astype(
        "uint16"
    )
    val_pred_reorder_long["pred_y"] = 1

    val_pred = up_pred_prob.merge(
        val_pred_reorder_long, on=["order_id", "product_id"], how="left"
    )
