...
df["up_purchase_ratio_r5"] = df["up_num_purchases_r5"] / df["user_max_order"].apply(
    lambda x: min(T, x)
)
df.fillna(0, inplace=True)

df[["user_id", "product_id", "up_num_purchases_r5", "up_purchase_ratio_r5"]].to_pickle(
    "{}/up_purchase_r5.pickle".format(data_folder)
)
