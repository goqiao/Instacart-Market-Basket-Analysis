...
up_purchase_time = pd.read_pickle('{}/days_since_last_purchase.pickle'.format(data_folder)).reset_index(drop=True)

key = ['user_id', 'product_id']
up_purchase_time = up_purchase_time.loc[~up_purchase_time._up_days_since_last_purchase.isnull()]
up_time = up_purchase_time.groupby(key).agg({'_up_days_since_last_purchase': [list, 'mean', trend_d1, skewness]}) 
